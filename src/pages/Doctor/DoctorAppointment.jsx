        ...
        {appointments.map((item, index) => {
          return (
            <div className={`flex flex-wrap justify-between max-sm:gap-2 sm:grid sm:grid-cols-[0.5fr_3fr_3fr_3fr_1fr_1fr] items-center text-gray-600 py-3 px-6 border-b hover:bg-gray-50 ${item.appointmentStatus == 0 ? "bg-white" : "bg-slate-100"}`} key={index}>
              ...
              {item.appointmentStatus == 2 ? <button className="text-sm text-red-500 text-left"> Canceled </button> :

                (new Date(`${item.date}T${item.time}`) > new Date() ? (<img className="w-10 cursor-pointer" src={assets.cancel_icon} alt="" />)
                  : (
                    <button className="text-sm text-stone-500 text-left">Expired</button>
                  ))}
